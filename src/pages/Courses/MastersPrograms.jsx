...
import mastersData from "../../data/MastersCourses.json";
...
const MastersPrograms = () => {
  ...
  useEffect(() => {
    ...
    console.log("Imported mastersData:", mastersData);
    if (
      !mastersData ||
      !mastersData.Faculties ||
      !Array.isArray(mastersData.Faculties)
    ) {
  ...
  if (error) {
    return (
      <div className="p-4 bg-red-100 text-red-800 rounded-lg">
        <p>Error: {error}</p>
        <p>Please check the Masters_Programs_Merged.json file in src/data/.</p>
      </div>
    );
  }

  if (!mastersData.Faculties || mastersData.Faculties.length === 0) {
    return (
      <div className="p-4 bg-yellow-100 text-yellow-800 rounded-lg">
        <p>No Masters programs found. Please verify the JSON data.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {mastersData.Faculties.map((division, index) => (
