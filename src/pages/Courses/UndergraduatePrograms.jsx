...
import undergraduateData from "../../data/Undergraduate_Programs_Merged.json";
...
const UndergraduatePrograms = () => {
  ...
  useEffect(() => {
    ...
    console.log("Imported undergraduateData:", undergraduateData);
    if (
      !undergraduateData ||
      !undergraduateData.Faculties ||
      !Array.isArray(undergraduateData.Faculties)
    ) {
  ...
  if (error) {
    return (
      <div className="p-4 bg-red-100 text-red-800 rounded-lg">
        <p>Error: {error}</p>
        <p>
          Please check the Undergraduate_Programs_Merged.json file in src/data/.
        </p>
      </div>
    );
  }
  ...
  if (
    !undergraduateData.Faculties ||
    undergraduateData.Faculties.length === 0
  ) {
    return (
      <div className="p-4 bg-yellow-100 text-yellow-800 rounded-lg">
        <p>No undergraduate programs found. Please verify the JSON data.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {undergraduateData.Faculties.map((division, index) => (
